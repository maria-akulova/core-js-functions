...
function getPolynom(...variables) {
  ...
  return (x) => {
    ...
    for (let i = 0; i < variables.length; i += 1) {
      result += variables[i] * x ** (variables.length - 1 - i);
    }
    return result;
  };
}
...
function retry(func, attempts) {
  let countAtt = attempts;
  return function recallFunc() {
    try {
      return func();
    } catch (error) {
      if (countAtt > 0) {
...
function logger(func, logFunc) {
  return (...passedParams) => {
    const funcWithArgs = `${func.name}(${passedParams
      .map((arg) => JSON.stringify(arg))
      .join(',')})`;
    logFunc(`${funcWithArgs} starts`);
    const result = func(...passedParams);
    logFunc(`${funcWithArgs} ends`);
    return result;
  };
}
